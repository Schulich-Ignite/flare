...
This blurb is to cover some of the changes needed in [main.py](main.py) required to get this working. We first need import our `Platform` class to replace our `platforms` list with a sprite group. We'll also remove our `add_platform` function because it's not doing much other than creating a platform and adding it to our list of platforms (and adding confusion). We'll shrink that down to a single line of code (per platform) that reads "Add a brand new platform to the platforms list". Remember to remove the `color` tuple!
```py
...
+ platforms.add(Platform(700, 650, 200, 25))  # Fourth thin platform added just for fun
```
